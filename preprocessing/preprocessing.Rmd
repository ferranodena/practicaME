...
# 0) Age at enrollement > 7
x <- to_numeric_safely(df$Age_at_enrollment)
flag <- flag | (!is.na(x) & x > 40)
...
# Es converteixen a factors ordinals. Definim un rang de nivells de 0 a 30.
# Qualsevol valor fora d'aquest rang es convertirà en NA.
ordinal_levels <- 0:30
ordinal_labels <- as.character(ordinal_levels)

# 1) Curricular_units_1st_sem_enrolled
if ("Curricular_units_1st_sem_enrolled" %in% names(df)) {
  x <- to_numeric_safely(df$Curricular_units_1st_sem_enrolled)
  df$Curricular_units_1st_sem_enrolled <- factor(x, levels = ordinal_levels, labels = ordinal_labels, ordered = TRUE)
}

# 2) Curricular_units_2nd_sem_enrolled
if ("Curricular_units_2nd_sem_enrolled" %in% names(df)) {
  x <- to_numeric_safely(df$Curricular_units_2nd_sem_enrolled)
  df$Curricular_units_2nd_sem_enrolled <- factor(x, levels = ordinal_levels, labels = ordinal_labels, ordered = TRUE)
}

# 3) Curricular_units_1st_sem_evaluations
if ("Curricular_units_1st_sem_evaluations" %in% names(df)) {
  x <- to_numeric_safely(df$Curricular_units_1st_sem_evaluations)
  df$Curricular_units_1st_sem_evaluations <- factor(x, levels = ordinal_levels, labels = ordinal_labels, ordered = TRUE)
}

# 4) Curricular_units_2nd_sem_evaluations
if ("Curricular_units_2nd_sem_evaluations" %in% names(df)) {
  x <- to_numeric_safely(df$Curricular_units_2nd_sem_evaluations)
  df$Curricular_units_2nd_sem_evaluations <- factor(x, levels = ordinal_levels, labels = ordinal_labels, ordered = TRUE)
}

# 5) Curricular_units_1st_sem_approved
if ("Curricular_units_1st_sem_approved" %in% names(df)) {
  x <- to_numeric_safely(df$Curricular_units_1st_sem_approved)
  df$Curricular_units_1st_sem_approved <- factor(x, levels = ordinal_levels, labels = ordinal_labels, ordered = TRUE)
}

# 6) Curricular_units_2nd_sem_approved
if ("Curricular_units_2nd_sem_approved" %in% names(df)) {
  x <- to_numeric_safely(df$Curricular_units_2nd_sem_approved)
  df$Curricular_units_2nd_sem_approved <- factor(x, levels = ordinal_levels, labels = ordinal_labels, ordered = TRUE)
}


# Variables binàries (credited, without_evaluations)
# Es converteixen a factors 0/1. Qualsevol valor > 0 es considera 1.

# 7) Curricular_units_1st_sem_credited
if ("Curricular_units_1st_sem_credited" %in% names(df)) {
  x <- to_numeric_safely(df$Curricular_units_1st_sem_credited)
  x_bin <- ifelse(is.na(x), NA, ifelse(x > 0, 1, 0))
  df$Curricular_units_1st_sem_credited <- factor(x_bin, levels = c(0, 1), labels = c("0", "1"), ordered = FALSE)
}

# 8) Curricular_units_2nd_sem_credited
if ("Curricular_units_2nd_sem_credited" %in% names(df)) {
  x <- to_numeric_safely(df$Curricular_units_2nd_sem_credited)
  x_bin <- ifelse(is.na(x), NA, ifelse(x > 0, 1, 0))
  df$Curricular_units_2nd_sem_credited <- factor(x_bin, levels = c(0, 1), labels = c("0", "1"), ordered = FALSE)
}

# 9) Curricular_units_1st_sem_without_evaluations
if ("Curricular_units_1st_sem_without_evaluations" %in% names(df)) {
  x <- to_numeric_safely(df$Curricular_units_1st_sem_without_evaluations)
  x_bin <- ifelse(is.na(x), NA, ifelse(x > 0, 1, 0))
  df$Curricular_units_1st_sem_without_evaluations <- factor(x_bin, levels = c(0, 1), labels = c("0", "1"), ordered = FALSE)
}

# 10) Curricular_units_2nd_sem_without_evaluations
if ("Curricular_units_2nd_sem_without_evaluations" %in% names(df)) {
  x <- to_numeric_safely(df$Curricular_units_2nd_sem_without_evaluations)
  x_bin <- ifelse(is.na(x), NA, ifelse(x > 0, 1, 0))
  df$Curricular_units_2nd_sem_without_evaluations <- factor(x_bin, levels = c(0, 1), labels = c("0", "1"), ordered = FALSE)
}
```

Generem valors entre [0,10) per a poder fer servir les variables Curricular_units_1st/2nd_sem_grade
```{r}
library(truncnorm)

# Definim els límits de la distribució uniforme
a <- 0
b <- 10

# 1. Càlcul de la mitjana teòrica
mitjana_teorica <- (a + b) / 2

# 2. Càlcul de la desviació estàndard teòrica
sd_teorica <- (b - a) / sqrt(12)

print(paste("Mitjana teòrica:", mitjana_teorica))
print(paste("Desviació estàndard teòrica:", sd_teorica))


imputa_valors_0_10 <- function(vec, mean_impute = mitjana_teorica, sd_impute = sd_teorica, lower = 0, upper = 10) {
  # Índex dels valors que actualment estan dins l'interval [0, 10)
  idx <- which(vec >= lower & vec < upper)
  
  n <- length(idx)
  if (n > 0) {
    # Genera valors truncats per aquest nombre d'elements
    new_vals <- rtruncnorm(n, a = lower, b = upper, mean = mean_impute, sd = sd_impute)
    # Substitueix només aquests valors a la posició corresponent
    vec[idx] <- new_vals
  }
  return(vec)
}

# Aplica-ho al dataframe així:
df$Curricular_units_1st_sem_grade <- imputa_valors_0_10(df$Curricular_units_1st_sem_grade)
df$Curricular_units_2nd_sem_grade <- imputa_valors_0_10(df$Curricular_units_2nd_sem_grade)

```


## 6. Generem QQ-plots finals per a comparar resultats.
...
```{r}
write.csv(df, "clean-data.csv", row.names = FALSE)
```
...
ExpReport(data = df, Target = "Target", op_file = "eda_obj_categ_inicial.html", Rc = NULL)
```

## 4. Apliquem normes de coherència i eliminem outliers

Aplicarem normes de coherència seguint el que té sentit dins de la nostra dataset i eliminarem outliers basant-nos en els resultats del informe inicial. També canviem variables numèriques a categòriques en base als resultats de l'anàlisi exploratori.
...
## 5. Generem informes finals per a comparar resultats.
...
## 6. Desem el dataset preprocessat
```{r}

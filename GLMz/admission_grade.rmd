...
```{r}
library(car)
residualPlots(model_poli)
```

```{r}
library(car)
influencePlot(model_poli)
```

```{r}
anova(model_step, model_poli, test='Chisq')
```
...
```{r}
residualPlot(model_poli_quasi)
```

```{r}
residualPlots(model_poli_quasi)
```

```{r}
influencePlot(model_poli_quasi)
```

```{r}
anova(model_step, model_poli_quasi)
```
...
# Provem a crear un model amb familia gamma, link log
```{r}
model_gamma <- glm(Admission_grade ~ . - Target, data = data, family = Gamma(link='log'))
summary(model)
```

```{r}
model_step_gamma <- step(model_gamma, direction = "both")
```

```{r}
AIC(model_step, model_step_gamma)
BIC(model_step, model_step_gamma)
